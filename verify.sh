...
echo "=== hn-cpp26-trivial-loop-boundary-lab verification ==="
python3 -m py_compile "$ROOT/evaluator.py" && echo "py_compile evaluator.py: OK"
python3 -m py_compile "$ROOT/tests/test_cpp26_trivial_loop.py" && echo "py_compile tests: OK"
...
if git -C "$ROOT" diff --quiet -- results.json RESULTS.md 2>/dev/null; then
  ...
  git -C "$ROOT" diff -- results.json RESULTS.md 2>/dev/null | head -n 200
fi
